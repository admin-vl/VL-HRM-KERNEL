...
import { Plus, Upload, Eye, Edit, Trash2, Lock, Unlock, MoreHorizontal, Key } from 'lucide-react';
...
if (hasPermission(permissions, 'create-employees')) {
  ...
  // ✅ Add Employee Button
  pageActions.push({
    label: planLimits && !canCreate
      ? t('Employee Create Limit Reached ({{current}}/{{max}})', {
    ...
    onClick: canCreate
      ...
      : () =>
          toast.error(
            t(
              'Employee limit exceeded. Your plan allows maximum {{max}} users. Please upgrade your plan.',
              { max: planLimits.max_users }
            )
          ),
    disabled: !canCreate
    ...
    // ✅ ✅ ✅ UPLOAD BULK EMPLOYEE BUTTON (PROPER WAY)
    pageActions.push({
        label: t('Bulk Upload '),
        icon: <Upload className="mr-2 h-4 w-4" />,
        variant: 'secondary', // ✅ Blue style
